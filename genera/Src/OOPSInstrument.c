...
void        tVocoderUpdate      (tVocoder* const v)
{
    ...
    v->gain = powf(10.0f, 2.0f * v->param[1] - 3.0f * v->param[5] - 2.0f);
    
    v->thru = powf(10.0f, 0.5f + 2.0f * v->param[1]);
    ...
    {
        v->f[0][12] = powf(10.0, -1.7 - 2.7f * v->param[4]); //envelope speed
        
        rr = 0.022f / v->nbnd; //minimum proportional to frequency to stop distortion
        for(i=1;i<v->nbnd;i++)
        {
            v->f[i][12] = (0.025f - rr * i);
            if(v->f[0][12] < v->f[i][12]) v->f[i][12] = v->f[0][12];
        }
        v->f[0][12] = 0.5f * v->f[0][12]; //only top band is at full rate
    }
    
    rr = 1.0 - powf(10.0f, -1.0f - 1.2f * v->param[5]);
    sh = powf(2.0f, 3.0f * v->param[6] - 1.0f); //filter bank range shift
...
void    tStifKarpSetStretch         (tStifKarp* const p, float stretch )
{
    ...
    for ( int i=0; i<4; i++ )
    {
        ...
        coefficient = -2.0f * temp * cosf(TWO_PI * freq / oops.sampleRate);
